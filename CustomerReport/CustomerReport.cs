...
namespace Erp.BasicEditorYulChon.CustomerEditor
{
    public partial class CustomerReport : UserControl
    {
        ...
        {
            Erp.CodeViewer.findCustomerForm fcf = new CodeViewer.findCustomerForm();
            fcf.ShowDialog();
            if (fcf.DialogResult == DialogResult.OK)
            {
                customerCode = fcf.customerCode;
                customerName = fcf.customerName;

                searchBox.Text = customerName;
                showData(customerCode);
            }
        }

        private void showData(string customerCode) // ?
        {
            this.customerDS.customers2.Clear();
            DataSet ds = Erp.BusinessManager.Customer.GetCustomerData(customerCode);
            this.customerDS.customers2.Merge(ds.Tables[0]);
            this.customerDS.customers2.AcceptChanges();
        }
        ...
        private void excelBtn_Click(object sender, EventArgs e)
        {
            ...
            if (this.fpSpread1.ActiveSheet.RowCount > 0)
                //저장할 엑셀의 파일명
                Excel(fpSpread1, "BOM현황" + DateTime.Now.ToShortDateString());
